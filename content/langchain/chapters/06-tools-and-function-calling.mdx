---
slug: 06-tools-and-function-calling
title: "Tools and Function Calling \u2014 Stop pretending the model picks for you."
subtitle: "Stop pretending the model picks for you."
order: 6
difficulty: mid
estMinutes: 35
track: langchain
...
mistakesPrevented: 7
objectives:
  - "Explain tool contracts without hand-waving."
  - "Choose when to small typed tools with boring result envelopes."
  - "Identify and avoid letting the model discover your production API by vibes."
  - "Trace the production risk before it becomes an incident."
diagrams:
  - lc-tool-call-roundtrip
  - lc-parallel-tool-execution
quiz:
  - q: "What is the safest first move when designing tool contracts?"
    options:
  ...
  - q: "Which anti-pattern does this chapter explicitly call out?"
    options:
      - "Returning a stack trace as a tool result and calling it a feature"
...
<Diagram id="lc-tool-call-roundtrip" caption="The production shape of tool contracts before the code starts freelancing." alt="Diagram showing tool contracts components and data flow" />
...
| --- | --- | --- |
| Primary move | small typed tools with boring result envelopes | letting the model discover your production API by vibes |
...
<Diagram id="lc-parallel-tool-execution" caption="How tool contracts behaves once state, observability, and failure paths are admitted into the room." alt="Second diagram for tool contracts showing execution and control flow" />
...
<HallOfShame tag="langchain" title="Returning a stack trace as a tool result and calling it a feature">
...
<RealTalk>
Production does not care that the demo was smooth. It cares whether the system can explain what happened, cap what it spends, resume after failure, and refuse dangerous work. Tool contracts earns its keep only when those controls are explicit.
</RealTalk>
