---
slug: 15-observability-langsmith
title: "Observability with LangSmith \u2014 Traces that save you at 3 AM, plus the self-hosting argument."
subtitle: "Traces that save you at 3 AM, plus the self-hosting argument."
order: 15
difficulty: boss
estMinutes: 40
track: langchain
...
mistakesPrevented: 8
objectives:
  - "Explain tracing and debugging without hand-waving."
  - "Choose when to trace every meaningful node with tags and run metadata."
  - "Identify and avoid debugging production by asking users to paste screenshots into Slack."
  - "Trace the production risk before it becomes an incident."
diagrams:
  - lc-trace-tree-anatomy
  - lc-langsmith-self-host-vs-cloud
quiz:
  - q: "What is the safest first move when designing tracing and debugging?"
    options:
  ...
  - q: "Which anti-pattern does this chapter explicitly call out?"
    options:
      - "Shipping prompts as Python f-strings, no versioning, no diffs, no remorse"
...
<Diagram id="lc-trace-tree-anatomy" caption="The production shape of tracing and debugging before the code starts freelancing." alt="Diagram showing tracing and debugging components and data flow" />
...
<Diagram id="lc-langsmith-self-host-vs-cloud" caption="How tracing and debugging behaves once state, observability, and failure paths are admitted into the room." alt="Second diagram for tracing and debugging showing execution and control flow" />
...
<HallOfShame tag="langchain" title="Shipping prompts as Python f-strings, no versioning, no diffs, no remorse">
...
<RealTalk>
Production does not care that the demo was smooth. It cares whether the system can explain what happened, cap what it spends, resume after failure, and refuse dangerous work. Tracing and debugging earns its keep only when those controls are explicit.
</RealTalk>
