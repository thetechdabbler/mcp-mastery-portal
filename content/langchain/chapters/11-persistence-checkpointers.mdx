---
slug: 11-persistence-checkpointers
title: "Persistence and Checkpointers \u2014 How to survive a pod restart without a customer noticing."
subtitle: "How to survive a pod restart without a customer noticing."
order: 11
difficulty: boss
estMinutes: 45
track: langchain
...
mistakesPrevented: 9
objectives:
  - "Explain durable graph state without hand-waving."
  - "Choose when to Postgres or Redis for production resumes with retention policy."
  - "Identify and avoid mistaking local memory for durability because the demo did not crash."
  - "Trace the production risk before it becomes an incident."
diagrams:
  - lc-checkpointer-thread-tree
  - lc-checkpoint-backends-tradeoffs
quiz:
  - q: "What is the safest first move when designing durable graph state?"
    options:
  ...
  - q: "Which anti-pattern does this chapter explicitly call out?"
    options:
      - "InMemory checkpointer in production. Just why."
...
<Diagram id="lc-checkpointer-thread-tree" caption="The production shape of durable graph state before the code starts freelancing." alt="Diagram showing durable graph state components and data flow" />
...
<Diagram id="lc-checkpoint-backends-tradeoffs" caption="How durable graph state behaves once state, observability, and failure paths are admitted into the room." alt="Second diagram for durable graph state showing execution and control flow" />
...
<HallOfShame tag="langchain" title="InMemory checkpointer in production. Just why.">
...
<RealTalk>
Production does not care that the demo was smooth. It cares whether the system can explain what happened, cap what it spends, resume after failure, and refuse dangerous work. Durable graph state earns its keep only when those controls are explicit.
</RealTalk>
