...
import { AGENTCORE_CHAPTERS_DIR, AGENTCORE_PLAYBOOK_PATH } from "@/lib/content-paths";
...
export async function collectAgentcoreDiagramIds(): Promise<DiagramEntry[]> {
  const out: DiagramEntry[] = [];
  const files = await fs.readdir(AGENTCORE_CHAPTERS_DIR);
  for (const file of files.filter((f) => f.endsWith(".mdx"))) {
    const full = path.join(AGENTCORE_CHAPTERS_DIR, file);
  ...
  try {
    ...
    collectIdsFromMdxContent(content, "playbook", "Multi-Agent Playbook", out);
  } catch {
    // optional
  }
  return out;
