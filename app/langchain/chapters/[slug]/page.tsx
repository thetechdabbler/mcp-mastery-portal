...
import { LangchainVersionBadge } from "@/components/langchain-version-badge";
import { PrevNext } from "@/components/prev-next";
import { QuizBlock } from "@/components/mdx/quiz-block";
import { TocSidebar } from "@/components/toc-sidebar";
import {
  getAdjacentLangchainChapters,
  getLangchainChapterBySlug,
...
export async function generateStaticParams() {
  const ch = await getAllLangchainChapters();
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `/api/og/${slug}?track=langchain` }],
    },
  };
}

export default async function LangchainChapterPage(props: Props) {
  const { slug } = await props.params;
  const doc = await getLangchainChapterBySlug(slug);
  if (!doc) notFound();
  const { content } = await compileLessonMdx(doc.body);
  const chapters = await getAllLangchainChapters();
  const { prev, next } = getAdjacentLangchainChapters(chapters, slug);
  const headings = extractHeadingsFromMdx(doc.body);

  return (
    <div>
      <ChapterTracker slug={doc.frontmatter.slug} title={doc.frontmatter.title} track="langchain" />
      ...
      {doc.frontmatter.subtitle ? <p className="mb-4 text-lg text-muted-foreground">{doc.frontmatter.subtitle}</p> : null}
      <LangchainVersionBadge fm={doc.frontmatter} />
      {doc.frontmatter.mistakesPrevented ? (
        <p className="mt-3 rounded-md border bg-muted/40 px-3 py-2 text-sm">
          ...
          <span className="font-semibold">{doc.frontmatter.mistakesPrevented}</span> common LangChain mistakes.
        </p>
      ) : null}
      <div className="mt-6 flex gap-10">
        <TocSidebar headings={headings} />
        <article className="prose prose-zinc max-w-none flex-1 dark:prose-invert">
          {content}
          <h2>References</h2>
          <ul>
      ...
      <PrevNext prev={prev} next={next} basePath="/langchain/chapters" />
    </div>
  );
}
