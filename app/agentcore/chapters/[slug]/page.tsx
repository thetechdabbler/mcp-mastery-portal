...
export async function generateStaticParams() {
  const ch = await getAllAgentcoreChapters();
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: `/api/og/${slug}?track=agentcore` }],
    },
  };
}

export default async function AgentcoreChapterPage(props: Props) {
  const { slug } = await props.params;
  const doc = await getAgentcoreChapterBySlug(slug);
  if (!doc) notFound();
  const { content } = await compileLessonMdx(doc.body);
  const chapters = await getAllAgentcoreChapters();
  const { prev, next } = getAdjacentAgentcoreChapters(chapters, slug);
  ...
  return (
    <div>
      ...
      <AgentcoreVersionBadge fm={doc.frontmatter} />
      {doc.frontmatter.mistakesPrevented ? (
        <p className="mt-3 rounded-md border bg-muted/40 px-3 py-2 text-sm">
          ...
          <span className="font-semibold">{doc.frontmatter.mistakesPrevented}</span> common multi-agent mistakes.
        </p>
      ) : null}
      ...
      <PrevNext prev={prev} next={next} basePath="/agentcore/chapters" />
    </div>
  );
}
