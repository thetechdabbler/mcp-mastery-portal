"""Offline citations — implement answer_query(query) -> {answer, citations: [{id, snippet}]}."""
...
def retrieve(query: str, *, top_k: int = 3) -> list[Doc]:
    scored = [(retrieval_score(query, d), d) for d in CORPUS]
    scored.sort(key=lambda x: x[0], reverse=True)
    picked = [d for s, d in scored[:top_k] if s > 0]
    return picked or [CORPUS[2]]
...
def answer_query(query: str) -> dict:
    pool = retrieve(query, top_k=3)
    ranked = rerank(query, pool)
    top = ranked[0]
    snippet = top["text"][:80].strip()
    if len(top["text"]) > 80:
        snippet += "…"
    answer = f"[{top['id']}] {snippet}"
    return {
        "answer": answer,
